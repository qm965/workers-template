...
worker.tool('extractFullDocument', {
	...
	schema: j.object({
		pageId: j.string().describe('要提取内容的 Notion 页面 ID（32位字符串，或页面 URL 末尾的 ID 部分）'),
		followChildPages: j.boolean().describe('是否递归进入子页面提取内容，默认为 false').nullable(),
	}),
